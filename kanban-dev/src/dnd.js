...
export default class DragManager {
	...
	drag = event => {
		...
		const dragArea = this.findElement(dragAreaClass);
		const dragItem = this.findElement(dragItemClass);
	...
	dragEnd = event => {
		...
		if (itemShadow && itemShadow.parentElement) {
			...
			itemShadow.parentElement.replaceChild(item, itemShadow);
		} else {
			item.setAttribute('style', 'none');
		}

		this.dragItem.item = null;
		this.dragItem.area = null;
		this.dragItem.shadow = null;
		this.dragItem.collisionItem = null;
	}
	...
	findElement = (...attributeValue) => {
		...
		for (let value of attributeValue) {
			for (; element && element !== document; element = element.parentElement) {
				if (element.classList.contains(value) || element.id === value) return element
			}
		}
	}
